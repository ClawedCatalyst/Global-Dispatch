...
class SearchShipmentView(views.APIView):
    def post(self, request, *args, **kwargs):
        ...
        if hash is None:
            return Response(
                {"detail": "PLease provide the hash for tracking the shipment"}
            )
        ...
        return Response(
            ShipmentSerializer(
                instance=shipment, context={"request": self.request}
            ).data,
...
class ShipmentApproval(ListAPIView):
    ...
    def get(self, request):
        if request.user.isAdmin == True:
            shipment = (
                Shipment.objects.filter().exclude(status="approved").order_by("-id")
            )
            serializer = self.serializer_class(shipment, many=True)
            return Response(serializer.data)
        return Response({"msg": "Not Admin"})
            ...
            if shipment.status == "Approved":
                ...
                if serializer.is_valid(raise_exception=True):
                    ...
                    try:
                        ...
                        pass
                    try:
                        destination_location = serializer.data["destination_country"][
                            -3:
                        ]
                    except:
                        pass
                    ...
                    try:
                        ...
                        expected_price = predict_price(
                            shipment.quantity, shipment.commodity.volume, distance
                        )
